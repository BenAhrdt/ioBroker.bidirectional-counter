    ...
    async loadDevices(context) {
        ...
        for (const [deviceId, deviceValue] of Object.entries(sortedDevices)) {
            ...
            const res = {
                ...
                hasDetails: true,
                color: 'white',
                backgroundColor: this.adapter.activeStates[this.transformId(deviceId)] ? 'primary' : 'black',
                icon: this.adapter.activeStates[this.transformId(deviceId)]
                    ? `/adapter/${this.adapter.name}/bidirectional-counter-white.png`
                    : `/adapter/${this.adapter.name}/bidirectional-counter-white-inactive.png`,
            };
            res.customInfo = {
                id: deviceId,
                schema: {
